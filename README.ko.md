...
| `/ui-sync` | name, url | **변경 감지**: 저장 시나리오를 현재 페이지와 대조해 갱신 후 재실행 |
...
## 🖥️ CLI / CI (MCP 클라이언트 불필요)

대화에서 만든 시나리오를 파이프라인에서 헤드리스로 재실행할 수 있다 — 같은
러너·같은 리포트에 exit code와 JUnit XML이 추가된다:
