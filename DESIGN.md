...
- `--parallel N` — 시나리오당 **서브프로세스 1개**로 격리 실행(각자 자기 세션 싱글톤을
  가지므로 공유 상태 리팩토링 불필요). 동시성은 N으로 제한.
...
기본 100, 0=무제한) 실행을 남기고 초과분의 리포트 파일과 **실행 스탬프가 붙은
스크린샷**을 삭제한다(스크린샷 태그가 실행별이라 파일명만으로 판별). 정리 실패는
try/except로 격리 — 방금 성공한 저장을 절대 깨지 않는다.
